...
PixelShader =
{
	...
	[[
		float4 main( VS_OUTPUT In ) : PDX_COLOR
		{
			...
			const float vAlphaBreakVal = 1.3f;	
			const float vPadding = 0.81f;	
			...
			vAlpha *= clamp(smoothstep(0.22f, 0.325f, vAlpha) * 100.0f, 1.0f, 50.0f); 
			return float4( 0.3f, 0.5f, 0.8f, vAlpha * 0.10 ); //return float4( 0.0f, 0.4f, 1.0f, vAlpha * 0.25 )
		}
	]]
}
